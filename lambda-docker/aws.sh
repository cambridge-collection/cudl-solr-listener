...
function handler() {
  ...
  # Unsupported events can have differently structured JSON within .Records[].body
  # Test first for an explicit TestEvent where .Records[].body.Records[] doesn't exist
  # It is necessary to chain the jq calls because the contents of .Records[].body is an escaped string.
  # The second jq call converts it into actual json before parsing.
	EVENTPROP=$(echo "$1" | jq -r '.Records[].body' | jq -r '.Event') 1>&2

	if [[ $EVENTPROP == 's3:TestEvent' ]]; then
		echo "WARN: ${EVENTPROP} received" 1>&2
	elif [[ "${EVENTPROP}" != 'null' || ! "${EVENTPROP}" ]]; then
	  # JQ will return 'null' (as a string) if the property doesn't exist we need to ensure that we ignore them
	  # in this conditional. Otherwise, output error because EVENTPROP contains some event that we don't support
		echo "ERROR: Unsupported event: ${EVENTPROP}" 1>&2
		return 1
	else
	  # Since no event information found for the first type of notification, check for events in
	  # .Records[].body.Records[]
		...
		WILDCARDS="([*?{}[]|])"
		if [[ "$JSON_FILE" =~ $WILDCARDS || $S3_BUCKET =~ $WILDCARDS ]]; then
			if [[ "$JSON_FILE" =~ $WILDCARDS ]]; then echo "ERROR: File not processed because wildcard character in filename" 1>&2; fi
			if [[ $S3_BUCKET =~ $WILDCARDS ]]; then echo "ERROR: File not processed because wildcard character in bucket name" 1>&2; fi
			return 1
		else
